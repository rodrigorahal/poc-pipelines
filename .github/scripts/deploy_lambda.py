...
def get_alias_current_version(function_name):
    response = lambda_client.get_alias(
        FunctionName=function_name, Name=f"{function_name}_alias"
    )
...
def create_deployment(
    application_name, deployment_group_name, current_version, target_version
):
    app_spec = {
        "version": 0.0,
        "Resources": [
    ...
    response = codedeploy_client.create_deployment(
        ...
        revision={
            "revisionType": "String",
            "string": {"content": json.dumps(app_spec)},
        },
    )
